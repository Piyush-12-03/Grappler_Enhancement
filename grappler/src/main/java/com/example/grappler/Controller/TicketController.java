...
class TicketController {
    ...
    public ResponseEntity<Tickets> getTicketsByUserIdAndticketId(@PathVariable Long id ,@PathVariable Long ticketId) {
        try {
            Tickets ticket = ticketService.getTicketsByUserIdAndTicketId(id, ticketId);
            if (ticket==null) {
                return new ResponseEntity<>(HttpStatus.NOT_FOUND);
            } else {
                logger.info("Retrieved ticket for User ID: {}", id);
                return new ResponseEntity<>(ticket, HttpStatus.OK);
            }
        } catch (Exception e) {
            logger.error("Failed to retrieve ticket for User ID: {}", id, e);
            throw new RuntimeException("Failed to retrieve the tickets.", e);
        }
    }
}
